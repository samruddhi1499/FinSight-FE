...
  // Calculate percent (can exceed 100)
  const rawPercent =
    monthlyGoalData.goal && monthlyGoalData.goal !== 0
      ? (monthlyGoalData.current / monthlyGoalData.goal) * 100
      : 0;
  // Show actual percent even if > 100
  const percent = Math.round(rawPercent);

  const strokeDashoffset = circumference * (1 - Math.min(percent, 100) / 100);

  // Determine color: red for exceeded OR negative, teal otherwise
  const exceeded = monthlyGoalData.current > monthlyGoalData.goal;
  const strokeColor = exceeded || monthlyGoalData.current < 0 ? '#e53e3e' : '#38b2ac'; // red if exceeded or negative
...
            Current Expense: ${Math.abs(monthlyGoalData.current).toFixed(2)}
          </div>
        )}
        {exceeded && (
          <div className="text-gray-600">
...
        )}
        <div className="text-gray-600">
          Goal: ${monthlyGoalData.goal.toFixed(2)}
        </div>
        <div className="text-gray-600">
          Current Savings: ${monthlyGoalData.currentSavings.toFixed(2)}
        </div>
      </div>
    </div>
  );
